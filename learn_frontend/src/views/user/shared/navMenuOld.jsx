...
function NavMenu(props) {
    ...
    const [opacMenu, setOpacMenu] = useState(0);
    const [opacCat, setOpacCat] = useState(0);
    ...
    const getGamificationLink = async () => {
        ...
        switch (import.meta.env.VITE_HOME_URL) {
            case 'https://learn.prd-culture.pmicloud.biz/':
                ...
                break;
            case 'https://learn.qas-culture.pmicloud.biz/':
                urlGamification = "https://learn-gamification.qas-culture.pmicloud.biz?account="+securityData.Security_UserAccount()
                break;
            case 'https://learn.dev-culture.pmicloud.biz/':
                urlGamification = "https://learn-gamification.dev-culture.pmicloud.biz?account="+securityData.Security_UserAccount()
                break;
            default:
                urlGamification = "#";
        }
        
        setState(state => ({ ...state, linkGamification: urlGamification }));
    }
                            ...
                            {state.sectionList.map((v, idx) =>
                                <li className="dropdown panel" key={idx}>
                                    ...
                                    v.id === 7 ?
                                        ...
                                        :
                                            v.id === 999 ?
                                            <a className="nav-link" href={`${state.linkGamification}`} rel="noopener noreferrer" target="_blank">
                                                {v.title}
                                            </a>
                                            :
        ...
        return (
            ...
            <header className={"header_wrap fixed-top hover_menu_style2 " + addStyle} id="header-nav">
                <style>
                    ...
                    .submenu, .menu-topics-lvl-2, .menu-topics-lvl-3{
                    ...
                    .submenu a, .submenu a:not([href]):not([tabindex]){
                        font-family: 'Poppins';
                        font-style: normal;
                        font-weight: 700;
                        font-size: 16px;
                    ...
                    .menu-topics-lvl-2 a,
                    .menu-topics-lvl-2 a:not([href]):not([tabindex]){
                        font-family: 'Poppins';
                        font-style: normal;
                        font-weight: 600;
                        font-size: 16px;
                    ...
                    .menu-topics-lvl-3 a,
                    .menu-topics-lvl-3 a:not([href]):not([tabindex]){
                        font-family: 'Poppins';
                        font-style: normal;
                        font-weight: 400;
                        font-size: 14px;
                    ...
                    .submenu a:active,
                    .submenu a:hover,
                    .submenu a:not([href]):not([tabindex]):active,
                ...
                <Collapse in={dropdown}>
                    <div className='row' id="menuTopics">
                        <div className="menu-topics-container col-lg-8" >
                            ...
                            <div className='row' style={{ padding: "1% 10%" }}>
                                <div className='col-lg-4 overflow-auto' style={{ overflow: "unset", maxHeight: "80vh" }}>
                                    <ul className="nav-topics">
                                        {state.sectionList.map((v, idx) =>
                                            <li className={state.currentTab === idx ? "active submenu" : "submenu"} key={idx}
                                                onClick={() => setState(state => ({
                                                    ...state, currentTab: idx, idSection: v.id,
                                                    opacMenu: opacMenu == 100 && currentSection == v.id || v.id == 7  ? setOpacMenu(0) : setOpacMenu(100),
                                                    currentSection: setCurrentSection(v.id), opacCat: setOpacCat(0)
                                                }))}
                                            >
                                                {
                                                v.id === 7 ?
                                                    <a href={`${routeAll.routeViewAll.page.path}?menu=${state.contentFromYourNetwork}`}>
                                                        {v.title}
                                                    </a>
                                                    :
                                                        v.id === 999 ?
                                                        <a href={`${state.linkGamification}`} rel="noopener noreferrer" target="_blank">
                                                            {v.title}
                                                        </a>
                                                            :
                                                            <a data-toggle="tab" role={"button"} style={{ color: currentSection == v.id && opacMenu == 100 ? "#034AFF" : "#000" }}>
                                                                {v.title}
                                                            </a>
                                                }
                                            </li>
                                ...
                                <div className='col-lg-4 overflow-auto' style={{
                                    opacity: opacMenu,
                                    overflow: "unset",
                                    maxHeight: "80vh"
                                }}>
                                    <ul className="nav-topics">
                                        {state.menuList.map((v, idx) =>
                                            ...
                                            <li className="menu-topics-lvl-2 submenu" key={idx}
                                                onClick={() => setState(state => ({ ...state, opacCat: opacCat == 100 && currentMenu == v.id ? setOpacCat(0) : setOpacCat(100), currentSection: setCurrentMenu(v.id) }))}

                                            >
                                                {
                                                    state.categoryList.filter(x => x.menu_id == v.id || x.menu_id == currentSection).length > 0 ?
                                                        <a data-toggle="tab" role={"button"} style={{ color: currentMenu == v.id && opacCat == 100 ? "#034AFF" : "#000" }}>
                                                            <div dangerouslySetInnerHTML={{
                                                                __html: v.title
                                                            }} />
                                                        </a>
                                                        :
                                                        <a href={getLinkMenuNCategory(v, 'menu')}
                                                        >
                                                            <div dangerouslySetInnerHTML={{
                                                                __html: v.title
                                                            }} />
                                ...
                                <div className='col-lg-4 overflow-auto' style={{
                                    ...
                                    <ul>
                                        {state.categoryList.map((x, idx_category) =>
                                            x.menu_id == currentMenu &&
                                            <li className="menu-topics-lvl-3 submenu" key={idx_category}>
                                                <a href=
                                                    {
                        ...
                        <div className='col-lg-4' style={{
                            ...
                            <div className='headerExplore'>
                                <h6 className='headerTitle text-white'>
                                    {defaultLang.lang.txtNavbarStartYourJourney} &nbsp;
                                    <a href={routeAll.routesUser.learningPage.path} className="text-white">
                                        {defaultLang.lang.txtClickHere}
                                    </a>
                                </h6>
                            </div>
                        </div>
