...
function Table(items){
    return(
        <table className="table table-hover">
            ...
            (item) =>
                <tr key={item.id}>
                    <td>{item.user_account}<br/>{item.user_name}</td>
...
function redeemReward(props){
    ...
    const getTotalPage =  async () => {
        if(submitFilter){
            ...
            let isi = await axiosLibrary.postData('awbRedeemReward/ListData',credentials);
    ...
    const handleExport = async (e) => {
        ...
        if (startDate == "" || endDate == "") {
            ...
            if(validateForm()){
                ...
                let response = await axiosLibrary.postDataFile("awbRedeemReward/ExportData", param);
                if(response.status === 200){            
                    const url = window.URL.createObjectURL(new Blob([response.data]));
                    const link = document.createElement('a');
                    link.href = url;
                    link.setAttribute('download', 'redeem_reward - report'+startDate+'.xlsx');
