    ...
    return(
        <table className="table table-hover">
            ...
            (item) =>
                <tr key={item.id}>
                    <td>{item.id}</td>
                    <td>{item.user_name} & {item.user_account}</td>
                    <td>{item.user_email}</td>
                    <td> {item.log_date} <br/> {item.access_device} </td>
                    <td>{item.log_type}</td>
                    <td>
                        {item.log_info} 
                        {
                            <div dangerouslySetInnerHTML={{
                                __html:
                                    item.log_type  == 'Article' ? 
                                    '<br/>'+item.category_title + '<br/>' + item.article_title 
                                    : '' 
                            }}/>
...
function ActivityLog(props){
    ...
    const getTotalPage = async () => {
        if(submitFilter){
            ...
            let isi = await axiosLibrary.postData('awbActivityLog/ListData',credentials);
    ...
    const handleExport = async (e) => {
        ...
        if (startDate == "" || endDate == "") {
            ...
            if(validateForm()){
                ...
                let response = await axiosLibrary.postDataFile("awbActivityLog/ExportData", param);
