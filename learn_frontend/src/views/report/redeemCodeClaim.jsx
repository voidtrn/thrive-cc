...
function redeemCodeClaim(props){
    ...
    const getTotalPage =  async () => {
        if(submitFilter){
            ...
            let isi = await axiosLibrary.postData('awbRedeemCode/ListDataClaim',credentials);
    ...
    const handleExport = async (e) => {
        ...
        if (startDate == "" || endDate == "") {
            ...
            if(validateForm()){
                ...
                let response = await axiosLibrary.postDataFile("awbRedeemCode/ExportDataClaim", param);
                if(response.status === 200){            
                    const url = window.URL.createObjectURL(new Blob([response.data]));
                    const link = document.createElement('a');
                    link.href = url;
                    link.setAttribute('download', 'redeem_code claim - report'+startDate+'.xlsx');
...
export default redeemCodeClaim;