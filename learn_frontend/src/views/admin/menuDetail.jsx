...
function MenuDetail(props){
    ...
    const getDetail= useCallback(async() =>{
        ...
        if(data.md5ID!== null){
            setEditData(true)
            let response = await axiosLibrary.postData('awbMenu/SelectData',data);
            if(response.status === 200){
                setItems(response.data.data)
                setFile(file_path+"menu/"+response.data.data.menu_image)
                setLoading(false)
            }else{
                alert(response);
                setLoading(false)
    ...
    const getListSection= useCallback(async() =>{
        const data = {
            ...
            let response = await axiosLibrary.postData('awbMenu/ListSection',data);
            if(response.status === 200){
                setListSection(response.data.data)              
            }else{
                alert(response);
            }
    ...
    const submit= async () =>{
        ...
        if(!cancelDelete){
            if(!deleteData){
                ...
                fd.append("set_as_default", items.set_as_default);
                ...
                if(IsFileAttached){
                    fd.append("menu_image", fileInput.current.files[0]);
                } 

                if(editData){
                    //for edit data
                    fd.append("id", items.id);
                    let responseJson = await axiosLibrary.postData("awbMenu/UpdateData", fd);
                    if(responseJson.status === 200){
                        alert("DATA HAS BEEN UPDATED");
                        history.push(routeAdmin.menu.path)
                    }else{
                        alert(responseJson);
                    }
                } else {
                    //for insert data
                    fd.append("section_id", items.section_id);
                    fd.append("user_created", user_id);
                    fd.append("sort_index", 0);
                    let responseJson = await axiosLibrary.postData("awbMenu/InsertData", fd);
                    if(responseJson.status === 200){
                ...
                let responseJson = await axiosLibrary.postData("awbMenu/DeleteData", parameter);
                if(responseJson.status === 200){
        ...
        <div className="panel panel-default">
            ...
            <div className="clearfix">
                <div className="panel-body">
                    <a className="float-end btn btn-default" href={routeAdmin.menu.path} label="Back to overview" data-ui-loader="">
                            ...
                            <Col sm={12}>
                                <Tab.Content animation="true">
                                    <Tab.Pane eventKey="#tab-0">
                                            ...
                                            {/* <input disabled name="hdnkey" value={items.section_id}/>    */}
                                            {
                                                editData?
                                                    <input type="text" id="usereditform-email" style={{width:"300px"}} className="form-control" disabled
                                                        name="section" value={items.section_title} aria-required="true" aria-invalid="false" />
                                                :
                                                <select value={items.section_id} required
                                                    onChange={handleInputChange.bind(this)} id="section_id" name="section_id" style={{width:"300px"}} className="form-control">
                                                    {editData ? null: <option value="">... Select this ...</option> }
                                                    {listSection.map(
                                                        (itemSection) =>
                                                        <option key={itemSection.id} value={itemSection.id}>{itemSection.title}</option>
                                                    )
                                                    }
                                                </select>
                                            }
                                        ...
                                        <div className="mb-3 field-usereditform-email required">
                                            <label className="form-label" htmlFor="usereditform-email">&nbsp;Home Preview <span style={{color:"#ff0404"}}>(*) </span>
                                                ...
                                                <ul className="file-upload-requirement">
                                                    ...
                                                    <li>
                                                    image resolution 80 x 80 pixels
                                                    </li>
                                                </ul>
                                            </label>
                                            <br/>
                                            <input type="file"  
                                                    name="menu_image" id="menu_image" size="80" accept="image/jpg,image/png,image/jpeg,image/gif" ref={fileInput} onChange={ajaxFileUploadImage.bind(this)} required={editData===false?true:false} />
                                            <br/>
                                            <img style={{width:"160px",height:"auto"}}  src={file}    alt="" onError={addDefaultSrc} />

                                            <br/>
                                            <span className='badge bg-primary' id="upload-name" name="upload-name">{items.menu_image===""? "images": items.menu_image}</span>
                                        ...
                                        <div className="mb-3 field-profile-country">
                                            <label className="form-label" htmlFor="profile-country">&nbsp;Type Page</label>
                                            <select id="profile-country" style={{width:"150px"}} className="form-control" 
                                        ...
                                        { editData?
                                            items.section_id == 2?
                                                ...
                                                <div className="mb-3 field-profile-country">
                                                    ...
                                                    <label className="form-label" htmlFor="profile-country">&nbsp;Home Appearance</label>
                                                    <select id="profile-country" style={{width:"150px"}} className="form-control" 
                                                        value={items.set_as_default} onChange={handleInputChange.bind(this)} required name="set_as_default" aria-invalid="false">
                                                        {/* {editData ? null: <option value="">... Select this ...</option> } */}
