...
function TopPicks(props){
    ...
    const getTotalPage = useCallback(async () => {
        ...
        let isi = await axiosLibrary.postData('awbTopPicks/ListData',credentials);
    ...
    const getDetail=async(param)=>{
        ...
        history.push({
            pathname: routeAdmin.topPicksDetail.path,
    ...
    const moveUp = async(id,sort_index)=>{
        ...
        let responseJson = await axiosLibrary.postData('awbTopPicks/MoveUp',param);
    ...
    const moveDown = async(id,sort_index)=>{
        ...
        let responseJson = await axiosLibrary.postData('awbTopPicks/MoveDown',param);
        if(responseJson.status===200){
            getData()
        }else{
            alert(responseJson.status)
    ...
    const deleteItem = async(id)=>{
        ...
        let responseJson = await axiosLibrary.postData('awbTopPicks/DeleteData',param);
        if(responseJson.status===200){
                    ...
                    <div className="panel-body">
                        ...
                        <div className="table-responsive">
                            <div className="pull-right">
                                <a className="pull-right btn btn-primary btn-sm tt" href={routeAdmin.topPicksDetail.path} ><i className="fa fa-plus aria-hidden="></i> Add new</a>  
                            </div>
                            ...
                            <div id="h182093w0" className="grid-view">
                                <div className="summary">Showing <b>{offset+1} - {limit*(activePage-1)+items.length}</b> of <b>{totalData}</b> records.</div>
                                    <Table items={items} edit={getDetail} moveUp={moveUp} moveDown={moveDown} totalActiveData={totalActiveData}
                                        deleteItem={deleteItem} totalData={totalData} loading={loading}/>
                            </div> 
                            {totalData > limit ?
