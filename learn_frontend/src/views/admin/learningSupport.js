        ...
        <table className="table table-hover">
            ...
            {items.map(
                (item) =>
                    <tr key={item.id}>
                        <td>{item.title}<br/>
                            <img  style={{width:"90px",height:"auto"}} src={file_path + 'learn_support/' + item.logo_image}  alt={item.logo_image} />
                        </td>
                        <td>{item.about_eng}</td>
...
function LearningSupport(props){
    ...
    const getDetail=async(param)=>{
        ...
        history.push({
            pathname: routeAdmin.learningSupportDetail.path,
    ...
    const moveUp = async(id,sort_index)=>{
        ...
        let responseJson = await axiosLibrary.postData('awbLearningSupport/MoveUp',param);
    ...
    const moveDown = async(id,sort_index)=>{
        ...
        let responseJson = await axiosLibrary.postData('awbLearningSupport/MoveDown',param);
        if(responseJson.status===200){
            getData()
        }else{
            alert(responseJson.status)
    ...
    const deleteItem = async(id)=>{
        ...
        let responseJson = await axiosLibrary.postData('awbLearningSupport/DeleteData',param);
        if(responseJson.status===200){
                    ...
                    <div className="panel-body">
                        ...
                        <div className="table-responsive">
                            <div className="pull-right">
                                <a className="pull-right btn btn-primary btn-sm tt" href={routeAdmin.learningSupportDetail.path} ><i className="fa fa-plus aria-hidden="></i> Add new</a>  
                            </div>

                            <div id="h182093w0" className="grid-view">
                                <div className="summary">Showing <b>{offset+1} - {limit*(activePage-1)+items.length}</b> of <b>{totalData}</b> records.</div>
                                    <Table items={items} edit={getDetail} moveUp={moveUp} moveDown={moveDown} totalActiveData={totalActiveData}
                                        deleteItem={deleteItem} file_path={file_path} totalData={totalData} loading={loading}/>
                            </div> 
                            {totalData > limit ?
