    ...
    return(
        <table className="table table-hover">
            ...
            {items.map(
                (item) =>
                    <tr key={item.id}>
                        <td style={{width:"150px"}}>{item._code}</td>
                        ...
                        <td style={{width:"150px"}}><span style={ item.navbar_active ? {} :{  color:"#ff0707" } }>{item.navbar_active ? 'active' :'inactive'}</span></td>
                        <td>
                            <a className="btn btn-primary btn-xs tt" onClick={props.edit.bind(this,item.id)} ><i className="fa fa-pencil"></i>&nbsp; edit</a> {" "}
                            {/* <a className="btn btn-danger btn-xs tt"  onClick={props.deleteItem.bind(this,item.id)} ><i className="fa fa-times">&nbsp; delete</i></a> */}
...
function Section(props){
    ...
    const getTotalPage = useCallback(async () => {
        ...
        let isi = await axiosLibrary.postData('awbSection/ListData',credentials);
    ...
    const getDetail=async(param)=>{
        ...
        history.push({
            pathname: routeAdmin.sectionDetail.path,
    ...
    const moveUp = async(id,sort_index)=>{
        ...
        let responseJson = await axiosLibrary.postData('awbSection/MoveUp',param);
    ...
    const moveDown = async(id,sort_index)=>{
        ...
        let responseJson = await axiosLibrary.postData('awbSection/MoveDown',param);
        if(responseJson.status===200){
            getData()
        }else{
            alert(responseJson.status)
