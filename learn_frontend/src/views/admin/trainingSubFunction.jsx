            ...
            {items.map(
                (item) =>
                    <tr key={item.id}>
                        ...
                        <td style={{width:"150px"}}>
                            <span>
                                <a className="btn btn-success btn-xs tt" onClick={props.users.bind(this, item.id, 'users')}>
...
function TrainingSubFunction(props){
    ...
    const getTotalPage = useCallback(async () => {
        ...
        let isi = await axiosLibrary.postData('awbTrainingSubfunction/ListData',credentials);
