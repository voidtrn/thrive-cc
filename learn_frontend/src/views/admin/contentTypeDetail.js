...
function ContentTypeDetail(props){
    ...
    const getDetail= useCallback(async() =>{
        ...
        if(data.md5ID!== null){
            setEditData(true)
            let response = await axiosLibrary.postData('awbContentType/SelectData',data);
    ...
    const submit= async (e) =>{
        ...
        if(!cancelDelete){
            if(!deleteData){
                ...
                fd.append("table_name", items.table_name);
                ...
                if(editData){
                    ...
                    let responseJson = await axiosLibrary.postData("awbContentType/UpdateData", fd);
                    if(responseJson.status === 200){
                        alert("DATA HAS BEEN UPDATED");
                        history.push(routeAdmin.contentType.path)
                    }else{
                        alert(responseJson);
                    }
                } else {
                    ...
                    fd.append("system_flag", 0);
                    let responseJson = await axiosLibrary.postData("awbContentType/InsertData", fd);
                    if(responseJson.status === 200){
                ...
                let responseJson = await axiosLibrary.postData("awbContentType/DeleteData", parameter);
                if(responseJson.status === 200){
        ...
        <div className="panel panel-default">
            ...
            <div className="clearfix">
                <div className="panel-body">
                    <a className="float-right btn btn-default" href={routeAdmin.contentType.path} label="Back to overview" data-ui-loader="">
            ...
            <div className="panel-body" style={cssTarget(loading)}>
                <form id="czfrom" onSubmit={submit} method="post" style={{display: "block"}} encType='multipart/form-data'>
                            ...
                            <Col sm={12}>
                                <Tab.Content animation="true">
                                    <Tab.Pane eventKey="#tab-0">
                                        ...
                                        <div className="form-group field-usereditform-username required">
                                            <label className="control-label" htmlFor="usereditform-username">&nbsp;Table Name</label>
                                            <input type="text" id="usereditform-username" style={{width:"80%"}} className="form-control" disabled
                                                name="table_name" value={items.table_name} onChange={handleInputChange}  aria-required="true" aria-invalid="false" />
                                            <div className="help-block"></div>
                                        </div>
                    ...
                    {(editData===false || items.system_flag === 1? null : <button className="btn btn-danger" name="btnDelete" onClick={DeleteConfirm.bind(this)} value="delete">Delete</button> )}       
