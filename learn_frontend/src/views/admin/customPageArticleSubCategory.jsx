...
function ArticleSubCategory(props){
    ...
    const [listSubCategoryNoLimit, setListSubCategoryNoLimit] = useState([])
    ...
    const getSubCategoryNoLimit = useCallback(async (categoryIdStr) => {
        const credentials = {
            limit: 1000,
            offset:offset,
            category:"",
        ...
        // alert(categoryId)
        let isi = await axiosLibrary.postData('awbSubCategory/ListData',credentials);
        setListSubCategoryNoLimit(isi.data.data)
        setLoading(false)
    })
    ...
    useEffect(() => {
        if(categoryId != 'null'){
            ...
            getSubCategoryNoLimit(categoryId)
        }else {
