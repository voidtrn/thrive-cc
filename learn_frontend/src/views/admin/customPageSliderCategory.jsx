            ...
            {items.map(
                (item) =>
                    <tr key={item.id}>
                        ...
                        <td>
                            {item.file_type != 'mp4'?
                                <img  style={{width:"90px",height:"auto"}} src={file_path + 'slider_custom_page/' + item.slider_video} alt={item.slider_video} ></img>
                                :
                                (item.slider_video)
                            }
                        </td>
...
function SliderCategory(props){
    ...
    const getDetail=async(param)=>{
        ...
        history.push({
            pathname: routeAdmin.customPageSliderCategoryDetail.path,
    ...
    const moveUp = async(id,sort_index)=>{
        ...
        let responseJson = await axiosLibrary.postData('awbSliderCustomPage/MoveUp',param);
    ...
    const moveDown = async(id,sort_index)=>{
        ...
        let responseJson = await axiosLibrary.postData('awbSliderCustomPage/MoveDown',param);
        if(responseJson.status===200){
            getData(categoryId)
        }else{
            alert(responseJson.status)
    ...
    const deleteItem = async(id)=>{
        ...
        let responseJson = await axiosLibrary.postData('awbSliderCustomPage/DeleteData',param);
        if(responseJson.status===200){
