            ...
            {items.map(
                (item) =>
                    <tr key={item.id}>
                        ...
                        <td style={{width:"150px"}}><span style={ item.flag_posted ? {} :{  color:"#ff0707" } }>{item.flag_posted ? 'posted' :'not posted'}</span></td>
...
function PushNotifAdmin(props){
    ...
    const getTotalPage = useCallback(async () => {
        ...
        let isi = await axiosLibrary.postData('awbNotifAdmin/ListData',credentials);
    ...
    const getDetail=async(param)=>{
        ...
        history.push({
            pathname: routeAdmin.pushNotifAdminDetail.path,
