            ...
            {items.map(
                (item) =>
                    <tr key={item.id}>
                        ...
                        <td >{item.claim_point}</td>
                        <td >
                            <img  style={{width:"90px",height:"auto"}} src={file_path + 'reward/' + item.reward_image}  alt={item.reward_image} />
                        </td>
...
function Reward(props){
    ...
    const getTotalPage = useCallback(async () => {
        ...
        let isi = await axiosLibrary.postData('awbReward/ListData',credentials);
    ...
    const getDetail=async(param)=>{
        ...
        history.push({
            pathname: routeAdmin.rewardDetail.path,
    ...
    const deleteItem = async(id)=>{
        ...
        let responseJson = await axiosLibrary.postData('awbReward/DeleteData',param);
                    ...
                    <div className="panel-body">
                        ...
                        <div className="table-responsive">
                            <div className="pull-right">
                                <a className="pull-right btn btn-primary btn-sm tt" href={routeAdmin.rewardDetail.path} ><i className="fa fa-plus aria-hidden="></i> Add new</a>  
                            </div>

                            <div id="h182093w0" className="grid-view">
                                <div className="summary">Showing <b>{offset+1} - {limit*(activePage-1)+items.length}</b> of <b>{totalData}</b> records.</div>
                                    <Table items={items} edit={getDetail} file_path={file_path}
                                        deleteItem={deleteItem} loading={loading}/>
                            </div> 
                            {totalData > limit ?
