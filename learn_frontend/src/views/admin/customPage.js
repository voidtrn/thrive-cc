...
import ArticleSubCategory from './customPageArticleSubCategory';
import SliderCategory from './customPageSliderCategory';
            ...
            {items.map(
                (item) =>
                    <tr key={item.id_sub_category}>
                        ...
                        <td>
                            <img  style={{width:"90px",height:"auto"}} src={file_path + 'sub_category/' + item.category_image} alt={item.category_image} ></img>
                        </td>
...
function SubCategory(props){
    ...
    const titleStr = routeAdmin.customPageSubCategory.pageName;
    ...
    const getData = useCallback(async (categoryIdStr, offsetNew) => {
        setLoading(true)
        
        const credentials = {
            limit: limit,
            offset:(offsetNew == undefined ? offset : offsetNew),
            category:"",
            categoryId:categoryIdStr,
            platform_id:platform_id
        };
        ...
        console.log("isi.data.data", isi.data.data)
    })
    ...
    const getDetail=async(param)=>{
        ...
        history.push({
            pathname: routeAdmin.customPageSubCategoryDetail.path,
    ...
    const handleBackToOverview = (pathname) => {
        switch(pathname) {
            case routeAdmin.customPageSubCategory.path:
                handleTabSelect('#tab-0')
                break;
            case routeAdmin.customPageArticleSubCategory.path:
                handleTabSelect('#tab-1')
                break;
            case routeAdmin.customPageSliderCategory.path:
