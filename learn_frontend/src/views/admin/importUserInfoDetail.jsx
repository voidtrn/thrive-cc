...
function ImportUserInfoDetail(props){
    ...
    const submit= async () =>{
        ...
        if(IsFileAttached){
            fd.append("userInfo_file", fileInput.current.files[0]);
        } 

        let responseJson = await axiosLibrary.postData("awbUserInfo/ImportData", fd);
        if(responseJson.status === 200){
            alert("DATA HAS BEEN IMPORTED, TOTAL RECORD : "+responseJson.data.data);
            history.push(routeAdmin.importUserInfo.path)
        }else{
            // alert(responseJson);
        ...
        <div className="panel panel-default">
            ...
            <div className="clearfix">
                <div className="panel-body">
                    <a className="float-right btn btn-default" href={routeAdmin.importUserInfo.path} label="Back to overview" data-ui-loader="">
                                ...
                                <Tab.Content animation="true">
                                    <Tab.Pane eventKey="#tab-0">
                                        <div className="form-group field-usereditform-email required">
                                            <label className="control-label" forHtml="usereditform-email">&nbsp;Template <span style={{color:"#ff0404"}}>(*)</span>
                                                ...
                                                <ul className="file-upload-requirement">
                                                    <li>
                                                        <a href={file_path+'import_user_info_template.xlsx'}>download</a>
                                                    </li>
                                                </ul>
                                        ...
                                        <div className="form-group field-usereditform-email required">
                                            ...
                                            <br/>
                                            <input type="file"  
                                                name="user_info_file" id="user_info_file" size="40" accept=".csv, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel"
