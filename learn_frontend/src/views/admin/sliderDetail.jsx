...
function SliderDetail(props){
    ...
    const getDetail= useCallback(async() =>{
        ...
        if(data.md5ID!== null){
            setEditData(true)
            let response = await axiosLibrary.postData('awbSlider/SelectData',data);
            if(response.status === 200){
                setItems(response.data.data)
                setFile(file_path+"slider/"+response.data.data.slider_video)
                setFileMobile(file_path+"slider/"+response.data.data.slider_video_mobile)
                setLoading(false)
            }else{
                alert(response);
                setLoading(false)
        ...
        if(!cancelDelete){
            if(!deleteData){
                ...
                if(editData){
                    ...
                    let responseJson = await axiosLibrary.postData("awbSlider/UpdateData", fd);
                    ...
                    let responseJson = await axiosLibrary.postData("awbSlider/InsertData", fd);
                    if(responseJson.status === 200){
                ...
                let responseJson = await axiosLibrary.postData("awbSlider/DeleteData", parameter);
                if(responseJson.status === 200){
    ...
    const getTotalActive = useCallback(async () => {
        ...
        let isi = await axiosLibrary.postData('awbSlider/ListData',credentials);
