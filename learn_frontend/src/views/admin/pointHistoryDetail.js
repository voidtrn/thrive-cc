...
function PointHistoryDetail(props){
    ...
    const submit= async () =>{
        ...
        if(IsFileAttached){
            fd.append("pointHistory_file", fileInput.current.files[0]);
        } 

        let responseJson = await axiosLibrary.postData("awbPointHistory/ImportData", fd);
        if(responseJson.status === 200){
            alert("DATA HAS BEEN IMPORTED, TOTAL RECORD : "+responseJson.data.data);
            setLoading(false)
            history.push(routeAdmin.pointHistory.path)
        }else{
            // alert(responseJson);
        ...
        <div className="panel panel-default">
            ...
            <div className="clearfix">
                <div className="panel-body">
                    <a className="float-right btn btn-default" href={routeAdmin.pointHistory.path} label="Back to overview" data-ui-loader="">
            ...
            <div className="panel-body">
                <form id="czfrom" onSubmit={validateFile} method="post" style={{display: "block"}} encType='multipart/form-data'>
                                ...
                                <Tab.Content animation="true">
                                    <Tab.Pane eventKey="#tab-0">
                                        <div className="form-group field-usereditform-email required">
                                            <label className="control-label" forHtml="usereditform-email">&nbsp;Template <span style={{color:"#ff0404"}}>(*)</span>
                                                ...
                                                <ul className="file-upload-requirement">
                                                    <li>
                                                        <a href={file_path+'import_point_history_template.xlsx'}>download</a>
                                                    </li>
                                                </ul>
                                        ...
                                        <div className="form-group field-usereditform-email required">
                                            ...
                                            <br/>
                                            <input type="file"  
                                                name="point_history_file" id="point_history_file" size="40" accept=".csv, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/vnd.ms-excel"
                    ...
                    <ButtonSubmitAdmin txt={"Import"} loading={loading}/>
