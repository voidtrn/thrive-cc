            ...
            {items.map(
                (item) =>
                    <tr key={item.id}>
                        ...
                        <td><img style={{ width:"160px", height:"auto" }} src={file_path+ "gallery/" + item.gallery_image}    alt={"gallery "+item.id} /></td>
...
function Gallery(props){
    ...
    const getTotalPage = useCallback(async () => {
        ...
        let isi = await axiosLibrary.postData('dialogueGallery/ListData',credentials);
    ...
    const getDetail=async(param)=>{
        ...
        history.push({
            pathname: routeAdmin.galleryDetail.path,
    ...
    const moveUp = async(id,sort_index)=>{
        ...
        let responseJson = await axiosLibrary.postData('dialogueGallery/MoveUp',param);
    ...
    const moveDown = async(id,sort_index)=>{
        ...
        let responseJson = await axiosLibrary.postData('dialogueGallery/MoveDown',param);
        if(responseJson.status===200){
            getData()
        }else{
            alert(responseJson.status)
                        ...
                        <div className="row">
                            ...
                            <div className="col-md-6">
                                <div className="pull-right">
                                    <a className="pull-right btn btn-primary btn-sm tt" href={routeAdmin.galleryDetail.path} ><i className="fa fa-plus aria-hidden="></i> Add new</a>  
                                </div>
                            </div>
                        </div>
                    </div>
