...
function UftFeatureDetail(props){
    ...
    const getDetail= useCallback(async() =>{
        ...
        if(data.md5ID!== null){
            setEditData(true)
            let response = await axiosLibrary.postData('uftFeature/SelectData',data);
            
            if(response.status === 200){
                setItems(response.data.data)
                setFile(file_path+"feature/"+response.data.data.feature_image)
            }else{
                alert(response);
            }
        }
        ...
        if(validateForm()){
            if(!cancelDelete){
                if(!deleteData){
                    ...
                    fd.append("application_name", items.application_name);
                    ...
                    if(IsFileAttached){
                        fd.append("feature_file", fileInput.current.files[0]);
                    } 
            ...
            <div className="panel panel-default">
                ...
                <div className="clearfix">
                    <div className="panel-body">
                        <a className="float-end btn btn-default" href={routeAdmin.uftFeature.path} label="Back to overview" data-ui-loader="">
                        ...
                        <div className="tab-content">
                            <div className="tab-pane active" data-tab-index="0" id="tab-0">
                                <div className="mb-3 field-usereditform-email required">
                                    <label className="form-label" htmlFor="usereditform-email">&nbsp;Feature Name <span style={{color:"#ff0404"}}>(*) </span></label>
                                    <input type="text" id="usereditform-email" style={{width:"75%"}} className="form-control" name="application_name" value={items.application_name||''} onChange={handleInputChange}   aria-required="true" aria-invalid="false"/>

                                    <div className="help-block"></div>
                                </div>

                                <div className="mb-3 field-usereditform-email required">
                                    <label className="form-label" htmlFor="usereditform-email">&nbsp;Hyperlink (optional, sample : http://www.google.co.id)</label>
                                ...
                                <div className="mb-3 field-usereditform-email required">
                                    <label className="form-label" htmlFor="usereditform-email">&nbsp;Feature Image <span style={{color:"#ff0404"}}>(*) max upload file size : 300 KB, image resolution : (1300 px * 400 px)</span></label>
                                    <input type="file"  
                                            name="feature_image" id="feature_image" size="40" accept="image/jpg,image/png,image/jpeg,image/gif" ref={fileInput} onChange={ajaxFileUploadImage.bind(this)} required={editData===false?true:false} />
                                    <img style={{width:"160px",height:"auto"}}  src={file}    alt="" onError={addDefaultSrc} />
                                <br/>
                                <span className='label label-primary' id="upload-name" name="upload-name"> {items.feature_image===""? "images": items.feature_image} </span>
...
export default UftFeatureDetail;