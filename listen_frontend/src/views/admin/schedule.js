    ...
    return(
        <table className="table table-hover">
            ...
            {items.map(
                (item) =>
                    <tr key={item.id} >
                        <td style={{width:250}}>{item.title}</td>
                        <td>{item.place}</td>
                        <td>{moment(item.schedule_date).format('dddd, MMMM Do YYYY') +" , "+item.schedule_time}</td>
                        <td>{item.total_participant}</td>
                        <td><img style={{width:75,height:"auto"}} src={file_path+ "schedule/" + item.schedule_image}  alt={"schedule-image-"+item.id} /></td>
...
function Schedule(props){
    ...
    const moveUp = async(id,sort_index)=>{
        ...
        let responseJson = await axiosLibrary.postData('dialogueSchedule/MoveUp',param);
    ...
    const moveDown = async(id,sort_index)=>{
        ...
        let responseJson = await axiosLibrary.postData('dialogueSchedule/MoveDown',param);
        if(responseJson.status===200){
            getData()
        }else{
            alert(responseJson.status)
                        ...
                        <div className="row">
                            ...
                            <div className="col-md-6">
                                <div className="pull-right">
                                    <a className="pull-right btn btn-primary btn-sm tt" href={routeAdmin.scheduleDetail.path} ><i className="fa fa-plus aria-hidden="></i> Add new</a>  
                                </div>
                            </div>
                        </div>
                    </div>
