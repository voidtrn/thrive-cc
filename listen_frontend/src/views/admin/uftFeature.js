    ...
    return(
        <table className="table table-hover">
            ...
            {items.map(
                (item) =>
                    <tr key={item.id}>
                        <td style={{ width:"30%" }}>{item.application_name}</td>
                        <td> <img  style={{ width:"160px", height:"auto" }} src={file_path+ "feature/" + item.feature_image}    alt="" />
                        </td>
                        <td>
...
function UftFeature(props){
    ...
    const getDetail=async(param)=>{
        ...
        history.push({
            pathname: routeAdmin.uftFeatureDetail.path,
                ...
                <div className="panel-body">
                    ...
                    <div className="table-responsive">
                        <div className="pull-right">
                            <a className="pull-right btn btn-primary btn-sm tt" href={routeAdmin.uftFeatureDetail.path} ><i className="fa fa-plus aria-hidden="></i> Add new</a>  
                        </div>

                        <div id="h182093w0" className="grid-view">
                            <div className="summary">Showing <b>{offset+1} - {limit*(activePage-1)+items.length}</b> of <b>{totalData}</b> records.</div>
                                <Table items={items} edit={getDetail} pathFile={file_path} loading={loading}/>
                        </div> 
