...
function ReportSubmittedInitiate(props){
    ...
    const handleExport = async (e) => {
        ...
        let response = await axiosLibrary.postDataFile("dialogueInitiate/FormExport", param);
        if(response.status === 200){            
            const url = window.URL.createObjectURL(new Blob([response.data]));
            const link = document.createElement('a');
            link.href = url;
            link.setAttribute('download', `report_initiate_${moment().format('YYYYMMDDHHmmss')}.xlsx`);
    ...
    const getTotalPage = useCallback(async () => {
        if(submitFilter){
            ...
            let isi = await axiosLibrary.postData('dialogueInitiate/ListData',credentials);
    ...
    const updateStatus = async (selectProp) => {
        ...
        let isi = await axiosLibrary.postData('dialogueInitiate/UpdateFlagCheckMark',param);
        if(isi.status===200){
            setSubmitFilter(true)
        }else{
                ...
                <form method="post" onSubmit={(e)=>{e.preventDefault();setSubmitFilter(true)}}>
                    <div className="filter">
                        <div className="search-form">
                            ...
                            <div className="pull-right" style={{position: "relative",top:"5px"}}>
...
export default ReportSubmittedInitiate;